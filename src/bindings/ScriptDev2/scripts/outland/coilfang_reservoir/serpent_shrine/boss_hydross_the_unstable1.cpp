...
struct MANGOS_DLL_DECL boss_hydross_the_unstableAI : public ScriptedAI
{
    ...
    instance_serpentshrine_cavern* m_pInstance;                          // the instance
    ...
    void Reset()
    {
       ...
       /* if (m_pInstance && !m_pInstance->m_lBeamHelpers.empty())
            for (GUIDList::iterator itr = m_pInstance->m_lBeamHelpers.begin(); itr != m_pInstance->m_lBeamHelpers.end(); ++itr)
                if (Creature* pBeamer = m_creature->GetMap()->GetCreature(*itr))
                {
                    pBeamer->CastSpell(m_creature, SPELL_BLUE_BEAM, true);
                    pBeamer->ForcedDespawn();
                }*/

        /*if (SpellEntry* TempSpellNovaH = (SpellEntry*)GetSpellStore()->LookupEntry(SPELL_BLUE_BEAM))
        {
            TempSpellNovaH->EffectImplicitTargetA[0] = 25;
            TempSpellNovaH->EffectImplicitTargetB[0] = 0;
        }*/
    }
