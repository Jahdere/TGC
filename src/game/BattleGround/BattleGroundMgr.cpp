...
void BattleGroundQueue::DropOldMatchIfCan(GroupQueueInfo* group)
{
	...
	if(!arenaTeam->GetFoughtTeamList().empty())
	{
		...
		uint32 timerNextDrop = (10 - 10) * 30000 + 30000;
...
void BattleGroundMgr::BuildBattleGroundStatusPacket(WorldPacket* data, BattleGround* bg, uint8 QueueSlot, uint8 StatusID, uint32 Time1, uint32 Time2, ArenaType arenatype, Team arenaTeam)
{
	...
	*data << uint32(StatusID);                              // status
	...
	case STATUS_IN_PROGRESS:                            // status_in_progress
		...
		*data << uint8(arenaTeam == ALLIANCE ? 1 : 0);  // arenaTeam -- Lua_GetBattlefieldArenaFaction (bool)
		break;
	default:
...
void BattleGroundMgr::BuildPvpLogDataPacket(WorldPacket* data, BattleGround* bg)
{
	...
	*data << uint8(type);                                   // type (battleground=0/arena=1)
	...
	else
	{
		...
		*data << uint8(bg->GetWinner() == ALLIANCE ? 1 : bg->GetWinner() == HORDE ? 0 : 2); // who won (ALLIANCE - HORDE - DRAW)
	}
